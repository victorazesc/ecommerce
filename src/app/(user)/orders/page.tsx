...
const UserOrders = async () => {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-[calc(100vh-91px)] gap-2 px-4">
            <h2 className="mb-6 text-4xl font-bold">SEM PEDIDOS AINDA</h2>
            <p className="mb-4 text-lg">Para visualizar seus pedidos você deve estar logado.</p>
            <Link
    ...
    if (orders === undefined || orders === null) {
        return (
            <div className="flex flex-col items-center justify-center w-full h-[80vh] gap-2 px-4">
                <h2 className="mb-6 text-4xl font-bold">SEM ORDENS DE COMPRA AINDA</h2>
                <p className="mb-4 text-lg">Para criar um pedido adicione um produto ao carrinho e compre!</p>
                <Link
                    className="flex font-medium	 items-center bg-[#0C0C0C] justify-center text-sm min-w-[160px] max-w-[160px] h-[40px] px-[10px] rounded-md border border-solid border-[#2E2E2E] transition-all hover:bg-[#1F1F1F] hover:border-[#454545]"
                    href="/"
                >
            ...
            {orders.orders.map((order: OrderDocument, index: number) => (
                <div key={index} className="w-full transition duration-150 border border-solid rounded border-border-primary bg-background-secondary hover:bg-color-secondary">
                    <Link href={`/orders/${order._id}?items=${order.products.reduce((total, product) => total + product.quantity, 0)}`} className='flex flex-col justify-between h-full gap-2 px-4 py-5'>
                        <h4 className='font-semibold'>{`${format(order.purchaseDate, 'dd LLL yyyy')} | R$${(order.total_price / 100).toFixed(2)} | Items: ${order.products.reduce((total, product) => total + product.quantity, 0)} `}</h4>
